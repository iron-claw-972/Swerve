...
public class Drivetrain extends SubsystemBase implements DrivetrainIO {
  ...
  // Here we calculate the theoretical maximum angular velocity. You can also replace this with a measured amount.
    public static final double MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND = MAX_VELOCITY_METERS_PER_SECOND /
        Math.hypot(Constants.drive.TRACKWIDTH_METERS / 2.0, Constants.drive.WHEELBASE_METERS / 2.0);
  ...
  // cause the angle reading to increase until it wraps back over to zero.
    ...
    private DrivetrainIOInputs m_drivetrainLogInputs = new DrivetrainIOInputs();

    public Drivetrain(DrivetrainIO ioType) {
   ...
   */
    ...
    public void periodic() {
        ...
        m_frontLeftModule.set(states[0].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * Constants.kMaxVoltage, states[0].angle.getRadians());
        m_frontRightModule.set(states[1].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * Constants.kMaxVoltage, states[1].angle.getRadians());
        m_backLeftModule.set(states[2].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * Constants.kMaxVoltage, states[2].angle.getRadians());
        m_backRightModule.set(states[3].speedMetersPerSecond / MAX_VELOCITY_METERS_PER_SECOND * Constants.kMaxVoltage, states[3].angle.getRadians());

        updateInputs(m_drivetrainLogInputs);
        Logger.getInstance().processInputs("Drivetrain", m_drivetrainLogInputs);

    }

    // Logging stuff from here on out
    @Override
    public void updateInputs(DrivetrainIOInputs inputs) {
        
        inputs.frontLeft_steerAngle = m_frontLeftModule.getSteerAngle();
        inputs.frontLeft_driveVelocity = m_frontLeftModule.getDriveVelocity();
